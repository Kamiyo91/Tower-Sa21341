...
namespace VortexTower.Sae.Cards
{
    public class DiceCardSelfAbility_DefStance_Sa21341 : DiceCardSelfAbilityBase
    {
        ...
        {
            if (unit.bufListDetail.HasBuf<BattleUnitBuf_DefStance_Sa21341>()) return;
        ...
        private static void ChangeDeckEnemy(BattleUnitModel unit)
        {
            ...
            if (!unit.bufListDetail.HasBuf<BattleUnitBuf_RedAura_Sa21341>() && unit.faction == Faction.Enemy)
                foreach (var cardId in VortexModParameters.DarkSaeDefDeck)
                    unit.allyCardDetail.AddNewCardToDeck(cardId);
            else
                foreach (var cardId in VortexModParameters.SaeDefDeck)
                    unit.allyCardDetail.AddNewCardToDeck(cardId);
